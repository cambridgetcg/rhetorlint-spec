/**
 * Claim Feedback shared projection
 *
 ...
 * separate lanes.
 *
 * This file has no Node, browser, transport, storage, or DOM capability. The
 * caller supplies the RhetorLint engine, rules, source provenance, SHA-256
 * implementation, and the one environment-specific Proxy check. Both the
 * Node CLI and browser worksheet project the same inert JSON through here.
 */
...
export const SCHEMAS = Object.freeze({
  ...
  packet: "claim-feedback.packet/0.2",
...
export function createClaimFeedbackProjection({
  analyze,
  toSignal,
  rules,
  method,
  ...
  if (typeof analyze !== "function" || typeof toSignal !== "function") {
    throw new TypeError("Claim Feedback projection needs analyze() and toSignal()");
  }
  if (!rules || typeof rules !== "object" || !Array.isArray(rules.rules)) {
    throw new TypeError("Claim Feedback projection needs one parsed rule pack");
  }
  if (!method || typeof method !== "object") {
    throw new TypeError("Claim Feedback projection needs exact source provenance");
  }
  if (typeof sha256 !== "function" || typeof isProxy !== "function") {
    throw new TypeError("Claim Feedback projection needs sha256() and isProxy()");
  }

  // Detach all supplied data before any asynchronous digest can yield. These
  // snapshots also keep a caller from changing method or rule provenance
  // between two text lanes in the same packet.
  const RULES = deepFreeze(inertSnapshot(rules, "$rules"));
  const METHOD = deepFreeze(inertSnapshot(method, "$method"));
  exactKeys(METHOD, ["engine", "signal_projection", "rules", "packet_projection"], "$method");
  for (const part of ["engine", "signal_projection"]) {
    exactKeys(METHOD[part], ["name", "version", "source_sha256"], `$method.${part}`);
    text(METHOD[part].name, `$method.${part}.name`, { max: 255 });
    text(METHOD[part].version, `$method.${part}.version`, { max: 64 });
    digest(METHOD[part].source_sha256, `$method.${part}.source_sha256`);
  }
  exactKeys(METHOD.rules, ["id", "version", "source_sha256"], "$method.rules");
  text(METHOD.rules.id, "$method.rules.id", { max: 255 });
  text(METHOD.rules.version, "$method.rules.version", { max: 64 });
  digest(METHOD.rules.source_sha256, "$method.rules.source_sha256");
  if (METHOD.rules.id !== RULES.id || METHOD.rules.version !== RULES.version) {
    throw new TypeError("$method.rules must identify the supplied rule pack");
  }
  exactKeys(METHOD.packet_projection, ["schema", "source_sha256"], "$method.packet_projection");
  if (METHOD.packet_projection.schema !== SCHEMAS.packet) {
    throw new TypeError(`$method.packet_projection.schema must equal ${SCHEMAS.packet}`);
  }
  digest(METHOD.packet_projection.source_sha256, "$method.packet_projection.source_sha256");
...
function inertSnapshot(value, path) {
...
async function digestGeneratedJson(value) {
...
function assertPlainJson(value, rootPath = "$input") {
  ...
  function visit(item, path, depth) {
    ...
    if (isProxy(item)) throw new TypeError(`${path} must not be a Proxy`);
...
async function assertInput(input) {
  ...
  if (await sha256(input.crawl.body_utf8) !== input.crawl.body_sha256) {
    throw new TypeError("$input.crawl.body_sha256 does not match body_utf8");
  }
  if (await sha256(input.claim.text) !== input.crawl.claim_sha256) {
  ...
  for (let index = 0; index < input.challenge.evidence.length; index += 1) {
    const evidence = input.challenge.evidence[index];
    ...
    if (await sha256(evidence.body_utf8) !== evidence.body_sha256) {
...
async function validateClaimFeedbackInput(input) {
  try {
    input = inertSnapshot(input, "$input");
    await assertInput(input);
    await makeKarmaDraft(input, await sha256(stableJson(input)));
    return [];
  } catch (error) {
...
function evidenceSummary(evidence) {
  return {
    ...
    body_bytes: utf8Bytes(evidence.body_utf8),
...
function wordingMethod() {
  return {
    ...
    signal_projection: { ...METHOD.signal_projection },
    rules: { ...METHOD.rules },
    packet_projection: { ...METHOD.packet_projection },
    language_support: "English pack only",
    offset_units: "UTF-16 code units",
  };
}

async function reviewOneText(value, language) {
  ...
  return {
    ...
    supplied_language: language,
    signal,
    analysis_result_sha256: await digestGeneratedJson(result),
    shared_signal_sha256: await digestGeneratedJson(signal),
...
async function wordingReview(input) {
  return {
    ...
    claim: await reviewOneText(input.claim.text, input.claim.language),
    challenge: await reviewOneText(input.challenge.text, input.challenge.language),
    response: input.response === null
      ? null
      : await reviewOneText(input.response.text, input.response.language),
...
async function makeKarmaDraft(input, inputDigest) {
  const crawlSource = [
    input.crawl.url,
  ...
  if (input.response !== null) {
    ...
    if (deedKind === "correction") {
      const replacementDigest = await sha256(input.response.replacement_claim);
      replyText = [
        input.response.text,
...
async function makeTrainingCandidate(input, digests) {
  const reasons = [];
  ...
  const reviewProposal = {
    ...
    corrected_claim_sha256: correction
      ? await sha256(correction.replacement_claim)
      : null,
...
async function responseProjection(response) {
  ...
  return {
    ...
    replacement_claim_sha256: response.replacement_claim === null
      ? null
      : await sha256(response.replacement_claim),
...
async function buildClaimFeedback(input) {
  input = inertSnapshot(input, "$input");
  await assertInput(input);
  const inputDigest = await sha256(stableJson(input));
  const response = await responseProjection(input.response);
  ...
  const base = {
    ...
    crawl_receipt: {
      ...
      body_bytes: utf8Bytes(input.crawl.body_utf8),
    ...
    wording_review: await wordingReview(input),
    challenge: {
    ...
    karma_draft: await makeKarmaDraft(input, inputDigest),
    training_candidate: await makeTrainingCandidate(input, requiredTrainingDigests),
    effects: {
      scope: "projection-call-only",
      network_requests: 0,
      messages_sent: 0,
      ...
      retries: 0,
      external_state_changes_by_projection: 0,
    },
    limits: [
  ...
  const packetSha256 = await sha256(stableJson(base));
  return {
    ...base,
    integrity: {
      ...base.integrity,
...
async function verifyClaimFeedbackPacket(packet, input) {
  packet = inertSnapshot(packet, "$packet");
  input = inertSnapshot(input, "$input");
  const expected = await buildClaimFeedback(input);
