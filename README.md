...
- **Claim correction** — the finite [Claim Feedback example](examples/claim-feedback/README.md) carries one exact claim and supplied crawl receipt through an evidence challenge, attributed reply or correction, an unsigned KARMA draft, and a held AI-training review proposal. Its [public Cloudflare worksheet](https://rhetorlint-claim-feedback.pages.dev/) runs the same deterministic projection in browser memory; the stronger stable-file adapter remains local. It crawls nobody, judges nobody, sends no claim bytes, and has no storage or persistent-write capability.
